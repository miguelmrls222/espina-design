// Abandoned Cart Recovery Worker — ejecutar cada 60 minutos
// Deploy como Worker separado con wrangler deploy, NO como Pages Function

// KV namespace: ABANDONED_CARTS
...
async function processAbandoned(env) {
  ...
  while (hasMore) {
    ...
    for (const session of body.data || []) {
      // Solo sesiones abiertas (no pagadas) con email
      ...
      // Verificar si ya enviamos email
...
async function enviarEmailRecuperacion(session, brevoKey) {
  ...
  const items = (() => {
    try { return JSON.parse(session.metadata?.cart || '[]') } catch { return [] }
  })()

  const listaItems = items.map(i =>
    `• ${i.n}${i.col ? ` (${i.col})` : ''} — $${(i.p || 0).toLocaleString('es-MX')} MXN × ${i.c || 1}`
  ).join('\n')
  ...
  try {
    const res = await fetch('https://api.brevo.com/v3/smtp/email', {
      ...
      body: JSON.stringify({
        sender: { name: 'Espina Design', email: 'no-reply@espinadesign.com' },
        to: [{ email }],
...
<p style="margin:6px 0 0">Espina Design — Hermosillo, Sonora, México</p>
</td></tr>
</table>
</body>
</html>`,
